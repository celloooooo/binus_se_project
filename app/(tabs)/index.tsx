import {
  ...
  Search,
  Target,
  User as UserIcon // ✅ ICON PROFIL FIX
  ,
...
export default function App() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed saving account');
    }
  };
          ...
          <TouchableOpacity style={styles.groupCard}>
            ...
            <View style={styles.groupInfo}>
              <View style={styles.groupRow}>
                ...
                />
                <View>
                  <Text style={styles.groupTitle}>BMW M owners</Text>
                  <View style={styles.groupMetaRow}>
                    <Text style={styles.groupMeta}>🌐 Public</Text>
                    <Text style={styles.groupMeta}>👥 166</Text>
                  </View>
                </View>
              </View>
            </View>
    ...
    <View style={styles.whiteContainer}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        <View style={styles.profileCoverWrapper}>
          <View style={[
            styles.profileCover, 
            { backgroundColor: currentUser?.coverImage ? 'transparent' : '#F5F5F5' }
          ]}>
            ...
            {currentUser?.coverImage && (
              <Image source={{ uri: currentUser.coverImage }} style={styles.profileCover} />
            )}
          </View>
        ...
        <View style={styles.profileStatsRow}>
          <View style={styles.profileStatItem}>
            <Text style={styles.profileStatNumber}>{currentUser?.stats.badges || 0}</Text>
            <Text style={styles.profileStatLabel}>Badges</Text>
          </View>
          <View style={styles.profileStatItem}>
            <Text style={styles.profileStatNumber}>{currentUser?.stats.posts || 0}</Text>
            <Text style={styles.profileStatLabel}>Posts</Text>
          </View>
          <View style={styles.profileStatItem}>
            <Text style={styles.profileStatNumber}>{currentUser?.stats.followers || 0}</Text>
            <Text style={styles.profileStatLabel}>Followers</Text>
          </View>
          <View style={styles.profileStatItem}>
            <Text style={styles.profileStatNumber}>{currentUser?.stats.following || 0}</Text>
        ...
        <View style={styles.profileContent}>
          ...
          {currentUser?.bio ? (
            ...
            <View style={styles.emptyBio}>
              <Text style={styles.emptyBioText}>Add bio to tell your story...</Text>
            </View>
          )}
            ...
            {currentUser?.cars?.length > 0 ? (
              currentUser.cars.slice(0, 1).map((car, index) => (
                <TouchableOpacity key={index} style={styles.vehicleCard}>
                  <Image source={{ uri: 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=1000' }} style={styles.vehicleImage} />
    ...
    <View style={styles.whiteContainer}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.discoverTagsContainer}>
          {['🔥 Trending Near Posts', '📌 Trending Hashtags', '🌍 Trending Worldwide Posts', '🎉 Today\'s Events', '🆕 New Events', '👋 Nearby Users', '👥 Popular Groups'].map((tag, index) => (
        ...
        {(!currentUser?.groups || currentUser.groups.length === 0) ? (
          ...
          <TouchableOpacity style={styles.groupCard}>
            <Image source={{ uri: 'https://images.unsplash.com/photo-1617531653520-4893f7bbf978?w=1200' }} style={styles.groupImage} />
            <View style={styles.groupInfo}>
              <View style={styles.groupRow}>
                <Image source={{ uri: 'https://upload.wikimedia.org/wikipedia/commons/4/44/BMW.svg' }} style={styles.groupLogo} />
                <View>
                  <Text style={styles.groupTitle}>BMW M Owners</Text>
                  <View style={styles.groupMetaRow}>
                    <Text style={styles.groupMeta}>🌐 Public</Text>
  ...
  if (!currentUser && (screen === 'home' || screen === 'profile' || screen === 'groups' || 
      screen === 'searchMain' || screen === 'addPost' || screen === 'inbox' || screen === 'notifications')) {
    setScreen('login');
    return <LoginScreen />;
  }
...
const styles = StyleSheet.create({
  ...
  emptyGroupsTitle: { 
    fontSize: 22, fontWeight: 'bold', marginTop: 20, color: '#333' 
  },
  emptyGroupsText: { 
    fontSize: 15, color: '#888', textAlign: 'center', marginTop: 8, 
    lineHeight: 22, marginBottom: 25 
  },
  joinGroupsBtn: { 
    backgroundColor: '#4A81D3', paddingHorizontal: 30, paddingVertical: 12, 
    borderRadius: 25 
  },
  joinGroupsBtnText: { 
  ...
  tabBar: { 
    ...
    backgroundColor: 'white', flexDirection: 'row', justifyContent: 'space-around', 
    alignItems: 'center', paddingBottom: 25, borderTopWidth: 0.5, borderTopColor: '#EEE' 
  },
  ...
  bottomProfile: { width: 32, height: 32, borderRadius: 16, borderWidth: 2, borderColor: '#2196F3' },
  ...
  emptyCoverText: { 
    marginTop: 8, color: '#999', fontSize: 14, fontWeight: '500' 
  },
  ...
  profileTopActions: { backgroundColor: 'white', borderRadius: 18, paddingHorizontal: 18, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', gap: 14 },
  profileFire: { fontSize: 20, fontWeight: '700', color: '#ff4da6' },
  profileAvatarWrapper: { 
    width: 140, height: 140, borderRadius: 70, borderWidth: 7, borderColor: 'white', 
    position: 'absolute', left: 20, bottom: -50 
  },
  profileAvatar: { width: 140, height: 140, borderRadius: 70 },
  emptyAvatar: { 
    width: 140, height: 140, borderRadius: 70, backgroundColor: '#F5F5F5', 
    justifyContent: 'center', alignItems: 'center' 
  },
  profileStatsRow: { marginTop: 35, flexDirection: 'row', justifyContent: 'space-evenly', paddingLeft: 150, paddingRight: 15 },
  ...
  emptyBio: { 
    marginTop: 20, padding: 20, backgroundColor: '#F8F9FA', borderRadius: 12, 
  ...
  vehicleCardsRow: { flexDirection: 'row', marginTop: 20, justifyContent: 'space-between' },
  vehicleCard: { width: width * 0.42, height: 210, borderRadius: 16, overflow: 'hidden', backgroundColor: '#DDD' },
  ...
  ownBadge: { position: 'absolute', top: 12, right: 12, backgroundColor: '#DDEEFF', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20 },
  ...
  showAllCard: { width: width * 0.42, height: 210, borderRadius: 16, backgroundColor: '#F2F2F2', justifyContent: 'center', alignItems: 'center' },
  showAllText: { fontSize: 24, fontWeight: 'bold', color: '#222', textAlign: 'center', lineHeight: 34 },
  vehicleTab: { marginTop: 30, backgroundColor: '#F2F2F2', alignSelf: 'flex-start', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 25 },
