...
export const GroupsScreen = ({
  ...
  return (
    <View style={styles.whiteContainer}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120, paddingTop: 15 }}>
        {loadingGroups ? (
          ...
          myGroups.map((group: any, index: number) => (
            ...
            >
              <Image source={{ uri: group.coverPhoto || group.cover_image || 'https://images.unsplash.com/photo-1603811478698-0b1d6256f79a?w=1200' }} style={styles.groupImage} />
                ...
                <View style={styles.groupRow}>
                  ...
                  <View>
                    ...
                    <View style={styles.groupMetaRow}>
                      <Text style={styles.groupMeta}>{group.isPrivate || group.visibility?.toLowerCase() === 'private' ? '🔒 Private' : '🌐 Public'}</Text>
                      <Text style={styles.groupMeta}>👥 {group.members || 1}</Text>
                    </View>
                  </View>
                </View>
              </View>
      ...
      <BottomBar active="groups" setScreen={setScreen} currentUser={currentUser} />
    </View>
  );
};
...
export const GroupFeedScreen = ({
  ...
  const dummyCover = selectedGroup.coverPhoto || selectedGroup.cover_image || 'https://images.unsplash.com/photo-1603811478698-0b1d6256f79a?w=1200';
  const dummyLogo = selectedGroup.logo || 'https://upload.wikimedia.org/wikipedia/commons/4/44/BMW.svg';
  ...
  return (
    <View style={styles.feedContainer}>
        ...
        <View style={styles.groupInfoContainer}>
          <View style={styles.avatarTagRow}>
            ...
            {(selectedGroup.tag || selectedGroup.tag_name) && (
              <View style={styles.groupTagBadgeDetailed}>
                <Text style={styles.groupTagTextDetailed}>{selectedGroup.tag || selectedGroup.tag_name}</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.groupDetailMetaRow}>
            <Text style={styles.groupDetailMetaText}>
              {selectedGroup.isPrivate || selectedGroup.visibility?.toLowerCase() === 'private' ? '🔒 Private' : '🌐 Public'}  👥 {selectedGroup.members || 1}
            </Text>
          </View>
          
          {(selectedGroup.bio || selectedGroup.description) && (
            <Text style={styles.groupDetailBioText}>{selectedGroup.bio || selectedGroup.description}</Text>
          )}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.groupTabsScroll}>
            <View style={[styles.groupTabPill, styles.groupTabActivePill]}>
              <Text style={styles.groupTabActiveText}>Info</Text>
            </View>
            <View style={styles.groupTabPill}>
              <Text style={styles.groupTabTextDetailed}>Group Garage</Text>
            </View>
            <View style={styles.groupTabPill}>
              <Text style={styles.groupTabTextDetailed}>Members</Text>
            </View>
          </ScrollView>
        </View>
            ...
            <View key={post.id || index} style={styles.postCard}>
              <View style={styles.postHeaderRow}>
                <View style={styles.postHeaderLeft}>
                  <Image source={{ uri: post.avatar || 'https://images.unsplash.com/photo-1542362567-b07e54358753?w=500' }} style={styles.postAvatar} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.postUserText}>
                      {post.user || 'User'} <Text style={styles.postGroupText}> {'>'} {selectedGroup.name}</Text>
                    </Text>
                    <Text style={styles.postLocationText} numberOfLines={1}>{post.location || 'Location'}</Text>
                  </View>
                </View>
              </View>
              ...
              <View style={[styles.postActionRow, { marginTop: post.images && post.images.length > 1 ? 12 : 16 }]}>
                <View style={styles.postActionLeft}>
                  ...
                  <TouchableOpacity style={styles.actionIcon} onPress={() => setShowGroupMenu(true)}>
      ...
      <TouchableOpacity style={styles.fabCreatePost} onPress={() => setScreen('createPost')}>
